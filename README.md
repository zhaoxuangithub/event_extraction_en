...
# 训练事件抽取模型
sh ./scripts/train.sh ace05_event
```
...
sh ./scripts/data/ace-event/collect_ace_event.sh D:/Work/data/ACE2005-TrainingData-V5.0
```
...
# 处理数据集, 里面包括 time识别
python ./scripts/data/ace-event/parse_ace_event.py default-settings --include_times_and_values
```
...
# trained a model named model.tar.gz
allennlp predict models/ace05_event/model.tar.gz \
    data/ace-event/processed-data/default-settings/json/test.json \
    --predictor dygie \
    --include-package dygie \
    --use-dataset-reader \
    --output-file predictions/event_predict.json \
    --cuda-device 0
```
